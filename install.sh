...
setup_env() {
    ...
    if [ -f "$HOME/.config/omarchy/env" ]; then
        if grep -q "OPENWEATHER_API_KEY" "$HOME/.config/omarchy/env"; then
            print_warning "API key already exists in ~/.config/omarchy/env"
        else
            echo "" >> "$HOME/.config/omarchy/env"
            echo "# OpenWeatherMap API Key for weather widget" >> "$HOME/.config/omarchy/env"
            echo "export OPENWEATHER_API_KEY=\"YOUR_API_KEY_HERE\"" >> "$HOME/.config/omarchy/env"
            print_status "Added API key placeholder to ~/.config/omarchy/env"
        fi
    else
...
EOF
        print_status "Created ~/.config/omarchy/env with API key placeholder"
    fi
...
test_installation() {
    ...
    if [ -f "$HOME/.config/omarchy/env" ] && grep -q "YOUR_API_KEY_HERE" "$HOME/.config/omarchy/env"; then
        print_warning "Please update your API key in ~/.config/omarchy/env"
...
main() {
    ...
    echo "2. Edit ~/.config/omarchy/env and replace YOUR_API_KEY_HERE"
    echo "3. Waybar will automatically show your weather"
    echo
